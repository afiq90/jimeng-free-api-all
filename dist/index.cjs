...
function md5Hex(input) {
  return (0, import_crypto2.createHash)("md5").update(input).digest("hex");
}
function md5Double(input) {
  const first = (0, import_crypto2.createHash)("md5").update(input).digest();
  return (0, import_crypto2.createHash)("md5").update(first).digest("hex");
}
...
function getXGnarly(queryString, requestBody, userAgent) {
  ...
  const md5Query = (0, import_crypto3.createHash)("md5").update(queryString).digest("hex");
  const md5Body = (0, import_crypto3.createHash)("md5").update(requestBody).digest("hex");
  const md5Ua = (0, import_crypto3.createHash)("md5").update(userAgent).digest("hex");
  const dataObj = {};
  const keyOrder = [];
...
var BrowserService = class {
  ...
  async ensureBrowser() {
    ...
    this.launching = (async () => {
      ...
      try {
        this.browser = await import_playwright_core.chromium.launch({
          headless: true,
          args: [
            "--no-sandbox",
...
async function initializeDatabase() {
  ...
  const pool2 = new import_pg.Pool({
...
function createSignature(method, url, headers, accessKeyId, secretAccessKey, sessionToken, payload = "") {
  ...
  let payloadHash = import_crypto4.default.createHash("sha256").update("").digest("hex");
  if (method.toUpperCase() === "POST" && payload) {
    payloadHash = import_crypto4.default.createHash("sha256").update(payload, "utf8").digest("hex");
...
---\u7ED3\u675F---`);
  ...
  const stringToSign = [
    ...
    timestamp,
    credentialScope,
    import_crypto4.default.createHash("sha256").update(canonicalRequest, "utf8").digest("hex")
  ].join("\n");
  logger_default.debug(`\u5F85\u7B7E\u540D\u5B57\u7B26\u4E32:
${stringToSign}`);
  const kDate = import_crypto4.default.createHmac("sha256", `AWS4${secretAccessKey}`).update(date).digest();
  const kRegion = import_crypto4.default.createHmac("sha256", kDate).update(region).digest();
  const kService = import_crypto4.default.createHmac("sha256", kRegion).update(service).digest();
  const kSigning = import_crypto4.default.createHmac("sha256", kService).update("aws4_request").digest();
  const signature = import_crypto4.default.createHmac("sha256", kSigning).update(stringToSign, "utf8").digest("hex");
...
function getProxyDispatcher() {
  ...
  if (proxyUrl) {
    try {
      _proxyDispatcher = new import_undici.ProxyAgent(proxyUrl);
...
function createSignature2(method, url, headers, accessKeyId, secretAccessKey, sessionToken, payload = "", awsRegion = "cn-north-1", serviceName = "imagex") {
  ...
  let payloadHash = import_crypto5.default.createHash("sha256").update("").digest("hex");
  if (method.toUpperCase() === "POST" && payload) {
    payloadHash = import_crypto5.default.createHash("sha256").update(payload, "utf8").digest("hex");
    headersToSign["x-amz-content-sha256"] = payloadHash;
  }
  const signedHeaders = Object.keys(headersToSign).map((key) => key.toLowerCase()).sort().join(";");
  const canonicalHeaders = Object.keys(headersToSign).sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase())).map((key) => `${key.toLowerCase()}:${headersToSign[key].trim()}
`).join("");
  ...
  const stringToSign = [
    ...
    timestamp,
    credentialScope,
    import_crypto5.default.createHash("sha256").update(canonicalRequest, "utf8").digest("hex")
  ].join("\n");
  const kDate = import_crypto5.default.createHmac("sha256", `AWS4${secretAccessKey}`).update(date).digest();
  const kRegion = import_crypto5.default.createHmac("sha256", kDate).update(region).digest();
  const kService = import_crypto5.default.createHmac("sha256", kRegion).update(service).digest();
  const kSigning = import_crypto5.default.createHmac("sha256", kService).update("aws4_request").digest();
  const signature = import_crypto5.default.createHmac("sha256", kSigning).update(stringToSign, "utf8").digest("hex");
  return `AWS4-HMAC-SHA256 Credential=${accessKeyId}/${credentialScope}, SignedHeaders=${signedHeaders}, Signature=${signature}`;
}
...
async function uploadImageForVideo(imageUrl, refreshToken, regionInfo) {
  ...
  try {
    ...
    const payloadHash = import_crypto5.default.createHash("sha256").update(commitPayload, "utf8").digest("hex");
    const commitRequestHeaders = {
      "x-amz-date": commitTimestamp,
      "x-amz-security-token": session_token,
      "x-amz-content-sha256": payloadHash
...
async function generateInternationalSeedanceVideo(_model, prompt = "", {
  ...
  for (const fieldName of imageFields) {
    ...
    const imageUri = imageFile ? await uploadImageBufferForVideo(import_fs2.default.readFileSync(imageFile.filepath), refreshToken, regionInfo) : await uploadInternationalImageUrl(imageUrl, refreshToken, regionInfo);
  ...
  for (const fieldName of videoFields) {
    ...
    const vodResult = videoFile ? await uploadMediaForVideo(import_fs2.default.readFileSync(videoFile.filepath), "video", refreshToken, videoFile.originalFilename, regionInfo) : await uploadInternationalVideoUrl(videoUrl, refreshToken, regionInfo);
...
async function _generateInternationalSeedanceVideoWithHistoryId(_model, prompt, {
  ...
  for (const fieldName of videoFields) {
    ...
    const vodResult = videoFile ? await uploadMediaForVideo(import_fs2.default.readFileSync(videoFile.filepath), "video", refreshToken, videoFile.originalFilename, regionInfo) : await uploadInternationalVideoUrl(videoUrl2, refreshToken, regionInfo);
    const entry = { idx: materialIdx++, type: "video", fieldName, videoResult: { vid: vodResult.vid, width: vodResult.width || 0, height: vodResult.height || 0, duration: vodResult.duration || 0, fps: vodResult.fps || 0 } };
    materialRegistry.set(fieldName, entry);
    if (videoFile == null ? void 0 : videoFile.originalFilename) registerAlias(videoFile.originalFilename, entry);
  }
...
function submitInternationalAsyncVideoTask(model, prompt, options, refreshToken) {
  ...
  if (!import_fs2.default.existsSync(ASYNC_TASK_DIR)) {
    import_fs2.default.mkdirSync(ASYNC_TASK_DIR, { recursive: true });
  }
...
var ASYNC_TASK_DIR = import_path6.default.join(process.cwd(), "tmp", "async-tasks");
...
function taskFilePath(taskId) {
  return import_path6.default.join(ASYNC_TASK_DIR, `${taskId}.json`);
}
function saveTaskToFile(task) {
  try {
    ...
    import_fs2.default.writeFileSync(taskFilePath(task.taskId), JSON.stringify(data, null, 2), "utf-8");
  } catch (err) {
...
function restoreTasksFromFiles() {
  try {
    ...
    const files = import_fs2.default.readdirSync(ASYNC_TASK_DIR).filter((f) => f.endsWith(".json"));
    if (files.length === 0) return;
    logger_default.info(`\u53D1\u73B0 ${files.length} \u4E2A\u5F02\u6B65\u4EFB\u52A1\u6587\u4EF6\uFF0C\u5F00\u59CB\u6062\u590D...`);
    for (const file of files) {
      const data = loadTaskFromFile(import_path6.default.join(ASYNC_TASK_DIR, file));
      if (!data) continue;
...
var videos_default = {
  ...
  post: {
    ...
    "/international/generations": async (request2) => {
      ...
      const hasKeyedUrlFields = Object.keys(request2.body || {}).some((key) => (key === "image_file" || key === "video_file" || key.startsWith("image_file_") || key.startsWith("video_file_")) && import_lodash16.default.isString(request2.body[key]));
      ...
      request2.validate("body.model", (v) => import_lodash16.default.isString(v) && allowedModels.includes(v)).validate("body.prompt", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isString(v)).validate("body.ratio", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isString(v)).validate("body.resolution", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isString(v)).validate("body.file_paths", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isArray(v)).validate("body.filePaths", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isArray(v)).validate("body.response_format", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isString(v)).validate("headers.authorization", import_lodash16.default.isString);
      const tokens = tokenSplit(request2.headers.authorization);
      const token = import_lodash16.default.sample(tokens);
      const {
        model,
      ...
      const finalDuration = import_lodash16.default.isUndefined(duration) ? isSeedance ? 4 : 5 : isMultiPart && typeof duration === "string" ? parseInt(duration) : duration;
      const finalRatio = import_lodash16.default.isUndefined(ratio) ? isSeedance ? "4:3" : "1:1" : ratio;
      const finalFilePaths = filePaths.length > 0 ? filePaths : file_paths;
      if (!import_lodash16.default.isFinite(finalDuration) || !Number.isInteger(Number(finalDuration))) {
    ...
    "/international/generations/async": async (request2) => {
      ...
      request2.validate("body.model", (v) => import_lodash16.default.isString(v) && allowedModels.includes(v)).validate("body.prompt", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isString(v)).validate("body.ratio", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isString(v)).validate("body.resolution", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isString(v)).validate("body.file_paths", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isArray(v)).validate("body.filePaths", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isArray(v)).validate("headers.authorization", import_lodash16.default.isString);
      const tokens = tokenSplit(request2.headers.authorization);
      const token = import_lodash16.default.sample(tokens);
      const {
        model,
    ...
    "/generations/async": async (request2) => {
      ...
      request2.validate("body.model", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isString(v)).validate("body.prompt", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isString(v)).validate("body.ratio", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isString(v)).validate("body.resolution", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isString(v)).validate("body.duration", (v) => {
        ...
        return import_lodash16.default.isFinite(v) && (v >= 4 && v <= 15 || v === 5 || v === 10);
      }).validate("body.file_paths", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isArray(v)).validate("body.filePaths", (v) => import_lodash16.default.isUndefined(v) || import_lodash16.default.isArray(v)).validate("headers.authorization", import_lodash16.default.isString);
      const tokens = tokenSplit(request2.headers.authorization);
      const token = import_lodash16.default.sample(tokens);
      const {
        model = DEFAULT_MODEL2,
